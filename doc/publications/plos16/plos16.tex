...
1~and~2), 
\marginpar{something wrong here. We do not report on chromosome 1 and 2.
Aidan} 
the time taken is excessive due to
the large amount of data being aggregated and processed by the single driver node during intermediate stages of building
the model (see Result~\ref{comp}).  This unbalanced work load where the driver node becomes the bottleneck and worker
...
In the first section we explore some of the properties of random forests, demonstrating that any potential flaws in
\cursedforest\ are an inherent property of random forests. Here we provide demonstrations using synthetic data.  We then
demonstrate the scalability of \cursedforest\ in respect to the dimensionality of the data, building a random forest
model on whole-genome data from the 1000 Genomes Project. Here we report on the accuracy (in terms of the ARI) 
and the error.  Finally, given the role different parameter values can play in model construction, we explore the effect tuning
these parameters can have on the prediction accuracy of the model.
...
The average prediction error for an individual tree $h(X; B)$ is
\marginpar{is this formulation correct in the case of classification? See Friedman on 0-1 losss, Rob}
\begin{equation}
...
GWAS data. What do we need to do in this area? What about linkage disequilibrium? Denis}
...
``DataFrame'', similar to DataFrames in R.  While Spark ML algorithms demonstrate some scalability when dealing with a large
number of samples, this scalability eventually break down as we include more features.
...
\marginpar{check details of simulation. Piotr}
Each dataset consists of $n$ samples and $m$ variables where $n << m$, and values for each variable are ordinal variables with
three levels represented as numbers $\{0, 1, 2\}$ (which correspond to an additive effect encoding of genomics variation) randomly
generated from uniform distribution with equal probabilities.  The dichotomous response is generated from a
Bernoulli trial with $p$ (probability of success) calculated by applying an inverse logit transformation to the continuous
...
\marginpar{replicated in methods section}
...
complex trait of multiple additive features.
...
Fig.~\ref{figure:synth}a illustrates that increasing the number of predictors sampled for splitting at each node
...
\marginpar{wrong figures cited. Rob}
Fig.~\ref{figure:synth}b illustrates that increasing \mtry\ also improves the OOB (out of bag) classification error.
...
%     forests with 100 trees trained on the synthetic dataset of 2.5M features and 5K samples.}  {\bf a)} Variable
%   importance of top 20 variables normalized to the sum of the linear model coefficients. {\bf Others} represents
%   aggregated importance of variables not present in the linear model. The dotted line represents the original linear
%   model coefficients.
%  {\bf b)} OOB (out of bag) classification error
%   estimate as function of \mtry\ fraction.}
%   \label{figure:synth}
% \end{adjustwidth}
% \end{figure}
...
We have demonstrated that using a different model of paralleization can extend the random forest to delaing with the
case of an extremely large number of variables. By comparing this implementation to other implentations (including those
optimized for large data sets) we have demonstrated the utility of this implentation.

We have treated the case of variable selection in a $p >> n$ model where most of the variables are uninformative. We
have also demostrated the utility of the model for large GWAs data sets.

\marginpar{signal rather than ``significant variables''}

Donoho and Tanner \cite{Donoho.and.Tanner.2009} give a ``universal phase change'' result based on arguments from
combinatorial geometry. This has applications in a large number of areas including variable selection in high
diensions. Consider figure \ref{figure:phase-diagram-equivalence.png} which shows the region where a model can recover
the important variables, plotted as a function of $\delta = n/p$ and $\rho =k/n$ (where $k$ is the number of significant
variables). There is a distinct doundary, shown empirically and also based on arguments from combinatorial geometry, to the
region where we can reliably recover significant variables. 

We note that the examples we are considering lie on the extreme left of the plot, close to the origin. So there are
limits to what can be recovered in the presence of noise. However in the case of data that is both big and wide, \cursedforest and
other VariantSpark methods may provide a useful tool.
