...
forests are also very easy to parallelize. As the forest is a sum of decision trees, it it possible to grow separate
...
genomic data have been developed in other programming languages. {\sc CloudForest}~\cite{Bressler2015} written in Go,
achieves fast run times by effective use of the CPU cache, optimizing for different classes of features and efficiently
multi-threading.  Similarly, {\sc Ranger}~\cite{Wright.and.Ziegle.2016} written in C++ with an R front-end is
...
\marginpar{check details of simulation. Piotr} Each dataset consists of $n$ samples and $p$ variables where $n << p$,
...
probabilities.  The dichotomous response is generated from a Bernoulli trial with $p$ (probability of success)
calculated by applying an inverse logit transformation to the continuous linear response ($y$) of five variables:
...
% results in a stable feature importance ranking.  This concept was originally proposed by Liaw and
...
\marginpar{wrong figures cited. Rob} Fig.~\ref{figure:synth}b illustrates that increasing \mtry\ also improves the OOB
