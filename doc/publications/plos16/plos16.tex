...
variance form. A number of different additive decompositions of classification error have been considered
in an effort to understand the success of bagging. They all less transparent than the regression
case (see \cite{Friedman.1997} and citations therein).
...
variables). There is a distinct doundary, shown empirically and also based on arguments from combinatorial geometry, to
the region where we can reliably recover significant variables.
...
Notes:
\begin{itemize}
\item \cite{Donoho.and.Stodden.2006} evalute variable selection by 
$$\frac{\Vert\hat{\beta}-\beta\Vert_2}{\Vert\beta\Vert_2}$$
However there is no $\hat{\beta}$ in a tree model. Piotr was using vaiable importace as
a measure of $\hat{\beta}$ (i.e. we expect varible importance to be proportional to $\hat{\beta}$. Is this correct?
