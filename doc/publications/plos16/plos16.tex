...
trees and use a high \mtry value which leads to greater accuracy (see the example in section \ref{section:synthetic}). 
...
\marginpar{this was done with n=10000. Needs to redone with n=2.5M and some things re-written}
Fig.~\ref{figure:synth1} illustrates the effect of varying \mtry and \ntree on these two measures. The default \mtry
value for a classification problem is $\sqrt{p}=100$ so in this case we are considering values considerably larger than
the default. Fig.~\ref{figure:out-of-bag-prediction-error-prod.png} shows the OOB error is minimized at the largerst
values of \mtry and \ntree considered  whereas Fig.~\ref{figure:rbo-prod.png} illustrates that the RBO measure is less
sensitive once we over a certain threshold.
...
We investigate the ability of using a random forest model we build on genomic data to identify variants related to a
specific phenotype.
In this case we aim to identify variants associated with Bone Minderal Density (BMD). We build a model on the BMD dataset
(see datasets) using a binary high or low BMD value as the label.
From this model, we output a list of the top 10,000 contributing variants, ranked by their Gini impurity. The variant with
the highest Gini impurity---and hence, importance---is ranked 1, and the variant with the lowest Gini impurity is ranked 10,000.
Then, using the 26 known and 6 novel BMD genes from Duncan et al., we compile a list of variants that are present in or near one of these
32 genes.

We then apply the Mann–Whitney U test to this list to verify that from our list, variants from known BMD loci are ranked
higher than variants from elsewhere in the genome.
We observe $P=1.3\times10^{-07}$, supporting our hypothesis that variants in close proximity to the known BMD genes are
contributing more information to our classification model.
...
We also frequently see highly-ranked variants in chromosome 22q11. Stagi et al. observe that individuals with a microdeletion
in this region have a significant in bone mass\cite{Stagi2010}. In this same region, we also observe variants present in
PRODH (proline dehydrogenase). PRODH catalyzes the first step in proline degredation, and as proposed by Phang et al., proline
is made available by collagen degradation \cite{Phang2008}.

DCDC5 (Doublecortin Domain Containing 5) is one of the 26 known genes, however it is one of the genes that Duncan et al.
failed to replicate in their study. DCDC5 does appear in our list of genes, however the first occurance of a variant in
or near this gene has a rank of 9,667 out of 10,000. Also, this gene is not highly expressed in bone, and any mechanisms
by which this gene might regulate BMD are unclear \cite{Thakker2012}. Although variants near DCDC5 are associated with
BMD \cite{Rivadeneira2009}, it is questionable as to whether the gene itself contributes to BMD.
...
than building a k-means model, but also unable to scale to as many dimensions (see Table~\ref{timingtable}). To keep
...
\begin{figure}[tbhp] 
    ...
    \begin{flushleft} 
      The figure shows the runtime for the 5 subsets of the 1000 genomes project and the different methods investigated. $\bigotimes$ marks the last dataset the different method completed before running out of memory or running more than 24 hours. The dashed lines mark the trajectory for how ranger and SparkML had performed given larger memory machines. 
    \end{flushleft}
\end{figure}
...
execution of 128 CPU cores. The results are visualized in figure \ref{figure:synth2} below (please note log scale on the
axes and the values on y axes are expresses as trees per hour).
...
\item A decision tree (random forest) is a heruistic search. It may recover a relationship in the space of the
  combinatorial search.
\end{itemize}

The existence of the  Donoho-Tanner phase transition is a salutory warning. There are likley to be limits, both
computionaal and logical, to the recovery of signals from noisy data. \cursedforest  is a contribution to addressing the
practivcal limits but the logical limits will still apply. However in the case of data that is both big and wide,
