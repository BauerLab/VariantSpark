...
specific phenotype.
In this case we aim to identify variants associated with Bone Mineral Density (BMD). We build a model on the BMD dataset
...
Then, using the 26 known and 6 novel BMD genes from Duncan~{\it et al.}, we compile a list of variants that are present in or near one of these
32 genes.
...
We also frequently see highly-ranked variants in chromosome 22q11. Stagi~{\it et al.} observe that individuals with a microdeletion
in this region have a significant in bone mass\cite{Stagi2010}. In this same region, we also observe variants present in
PRODH (proline dehydrogenase). PRODH catalyzes the first step in proline degradation, and as proposed by Phang~{\it et al.}, proline
is made available by collagen degradation \cite{Phang2008}.

DCDC5 (Doublecortin Domain Containing 5) is one of the 26 known genes, however it is one of the genes that Duncan~{\it et al.}.
failed to replicate in their study. DCDC5 does appear in our list of genes, however the first occurrence of a variant in
