...
this, we perform feature selection on over 7.2 million genomic variants to identify the locations associated with Bone
...
BMD-associated genes are enriched toward the top of our ranked variant list. To form this list of "known genes", we start
with the 26 known BMD genes listed by Duncan {\it et al.}~\cite{Duncan.2011}. We also include the 6 genes that they find
with a new BMD association. We then add to this list the genes that Estrada {\it et al.} found with a new BMD
association~\cite{Estrada2012}. In total, this gives us a list of 100 different genes.

We label variants from our ranked list that fall within 100 kb of these 100 genes and observe that they are ranked
...
We therefore investigate the highest-ranked features that are not located near genes from our known-gene list. One gene,
COLEC10	 (collectin 10) contains variants more recently associated with
