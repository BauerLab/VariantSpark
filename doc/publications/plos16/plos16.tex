...
than $2^{31}-1$), and the \textsc{randomForest} implementation in R lending itself to parallelization to grow multiple
...
\marginpar{make consistent capitalization for ranger, randomForest}
...
trees and use a high \mtry value which leads to greater accurace (see section \ref{section:synthetic}). 
...
essentially a collection of elements. In the case of Spark ML, each element in our RDD is a sample. RDDs contribute to
the scalability of Spark as they can distributed across multiple nodes and operated on in parallel. Even as we add more
samples to a dataset, Spark can still simply schedule extra tasks to handle the additional items in the RDD.
...
\marginpar{not clear. Are RDDs the problem or the solutuion Aidan: Better?}
...
The current implementation of \cursedforest\ uses an ``Gini impurity''
criteria for splitting. Let $f_q$ be the fraction of items labelled
with value $q$ where $q = 1, \ldots, Q$ at a node. The Gini impurity is
\[
I_G(f) = \Sigma_{q = 1}^Q f_q ( 1 - f_q ), 
\]
...
\begin{align*}
z=& \Sigma_{i=1}^{i=5} {w_i x_i} \\
\intertext{and}
\sigma_\epsilon^2 =& Var(z)(1- 0.125)/0.125 \\
\intertext{so}
y =&  z+\epsilon \textrm{ where } \epsilon \sim  N(0, \sigma_\epsilon^2).
\end{align*}
...
number of cores used, \cursedforest displays approximately linear dependency between the execution time and \mtry
(Fig. \ref{figure:synthetictiming.a}).
...
[REF] 
\marginpar{have we lost this table?}
and visualized in figure \ref{figure:synth2} below (please note log scale on the axes and the values on y axes are
