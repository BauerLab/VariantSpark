...
specific phenotype.  In this case we aim to identify variants associated with Bone Mineral Density (BMD). We build a
...
and hence importance, is ranked 1, and the variant with the lowest Gini impurity is ranked 10,000.  Then, using the 26
known and 6 novel BMD genes from \cite{Duncan.2011}, we compile a list of variants that are present in or near one of
these 32 genes.

We then apply the Mann–Whitney U test to this list to verify that from our list, variants from known BMD loci are ranked
higher than variants from elsewhere in the genome.  We observe $P=1.3\times10^{-07}$, supporting our hypothesis that
variants in close proximity to the known BMD genes are contributing more information to our classification model.
...
We also frequently see highly-ranked variants in chromosome 22q11. \cite{Stagi.2010}, observe that individuals with a
microdeletion in this region have a significant in bone mass \cite{Stagi.2010}. In this same region, we also observe
variants present in PRODH (proline dehydrogenase). PRODH catalyzes the first step in proline degradation, and as
proposed by \cite{Phang.2008}, proline is made available by collagen degradation \cite{Phang.2008}.

DCDC5 (Doublecortin Domain Containing 5) is one of the 26 known genes, however it is one of the genes that \cite{Duncan.2011}.
