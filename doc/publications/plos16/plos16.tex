...
\begin{itemize}
  ...
  \item are insensitive to uninforamtive predictors;
...
data sets are independently drawn. with replacement,  from the joint distribution of $(X,Y)$ so that each set comprises $n$ $(p+1)$-tuples
 $(x_1,y_1),\ldots, (x_n,y_n)$. This si done $B$ times and  a tree is grown on each of these $B$ samples.
...
compared to the total number of variables, then the rate of convergence will be quicker. It is not clear waht the size
...
VariantSpark made use of Spark's machine learning algorithms ``Spark ML''. These algorithms require data
