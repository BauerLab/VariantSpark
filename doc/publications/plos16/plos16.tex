...
To manage Developed by Apache, Hadoop is one platform for dealing with large data sets is 
...
However, we observe that Spark ML's random forests implementation isn't able to handle the 1000 Genomes data, and is unable to build a model. The reason for the failure can be attributed to the "curse of dimensionality". Where Spark ML
is designed to process a huge number of samples with a modest dimensionality, the 1000 Genomes data instead contains a huge number of dimensions (features). As a result, the default random forests implementation is unable to partition the data into small-enough tasks
for nodes on the computer cluster to handle.

Therefore, we develop our own 'wide' version of these ML algorithms.
