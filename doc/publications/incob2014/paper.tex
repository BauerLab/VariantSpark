...
Pre-processing the data to Mahout's sequence files took approximately 21 minutes (1255s) and used approximately 1.8GB (1831160kb) physical memory (pmem) and 7.6GB (7646232kb) virtual memory (vmem). Conversly,  pre-processing
the data to R's in memory matrix took approximately 56 minutes (3341s) and used approximately 17.7GB (17696940kb) pmem and 18GB (18000196kb) vmem.	
...
The clustering steps are the same for each sample, however, due to the increased feature size we designate more memory to the
YARN containers. For the first step, we specify \texttt{k}, the number of clusters. The function \texttt{RandomSeedGenerator.buildRandom}
creates a new sequence file containing \texttt{k} random centroids from the samples sequence file. The sequence file
