...
To demonstrate \variantSpark's capability, we cluster variant datasets from the 1000 Genomes Project~\cite{1KG2012} to determine population structure using the \kMeans{} clustering algorithm available in MLlib's. 
In the first section we benchmark \variantSpark's performance and accuracy against ADAM and a Hadoop Mahout implementation as well as more traditional methods (R, Python) and the purpose-build tool {\sc Admixture}~\cite{Alexander2009} limiting the used data to chromosome 22.
...
This increased pre-processing time is due to the Python and R implementations not natively supporting multithreading, while \variantSpark{}, ADAM and Hadoop can use the eight available cores.
The GATK-based VCF to PED conversion is also slower with over 10 minutes. 

Clustering the samples is also fastest in \variantSpark{} (1min and 20secs, see Table 1), despite \variantSpark{} and ADAM both using {\sc Spark}'s MLlib \kMeans{} implementation. % 80
...
Despite this, both are an order of magnitude slower than \variantSpark{} and ADAM, with Python requiring 11mins and 29secs and R requiring 7mins and 25secs.
...
We also investigate the cluster quality for the six different methods by comparing the annotated super-population label (AMR, AFR, EAS, SAS) for each individual in the 1000 Genomes data to the assigned label from the clustering allowing four clusters. 
%TODO: 0 seems not to be the lowest number as we get a negative score for PGP
...
\variantSpark{} utilises {\sc Spark}, which allows for in-memory cashing and hence performs 85\% faster than Hadoop (29 min).
