...
Unlike \variantSpark{} or our Hadoop implementation, which pre-process VCF files directly, ADAM requires the input to be in their ADAM file format, and VCF files must be converted to this binary format. 
...
This substantially different runtime for the pre-processing is due to the Python and R implementations not natively supporting multithreading while \variantSpark{}, ADAM and Hadoop are using 8 cores.
...
The increased speed compared to Hadoop is due the dataset (chromosome 22) being small enough to fit into memory. This enables Python and R to store the output from each \kMeans{} iteration directly in memory. Hadoop, however, is designed to write the entire output of each iteration to disk.
