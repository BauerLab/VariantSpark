...
\newcommand{\FullPhaseone}{phase3\_chr1-22}
...
Although the overall Mahout pipeline is about 40 minutes faster than R's, Mahout's k-means clustering takes two orders of magnitude longer. 
...
R, on the other hand, already has the entire matrix in-memory, on a single machine. This should speed up the clustering significantly as no disk-access or networking is required. However, the amount of memory available on a single machine will quickly become a limiting factor. 
...
$^{n \times m \times 8bytes}/_{1GB}$, where $n \times m$ are the matrix dimensions, 8bytes is the memory a double occupies (i.e. each matrix element), and  $1GB$ is $10^9$bytes.

% DB: not clear you need to spell out your argument a bit more...
% AO: Fixed maybe
The primary benefit of Hadoop is its scalability. This is achieved through MapReduce jobs which allows the task at hand to be separated into 'containers'.
...
%AO: Fixed nearly
In this section we demonstrate Mahout's scaling ability by increasing the numbers of features (variants per sample) input to our algorithm to include the 79 Million variants from the entire phase3 dataset (\FullPhasethree{}). 
...
This significant increase in time required is in part due to the larger container size. With each container consuming 6GB of memory, only up to six containers can run simultaneously, as opposed to 24 1.5GB containers from the smaller examples.
...
& & map & reduce  & map & reduce  & pre-processing & clustering & \\\midrule
        ...
        & \SevenPhaseone{} & 1536 & 3072 & 4096 & 4096  & 43m 56s & 5h 44m & 0.716\\
...
\centerline{\includegraphics[type=pdf,ext=.pdf,read=.pdf, scale=0.40]{images/signature}}
        \label{fig:sign}
