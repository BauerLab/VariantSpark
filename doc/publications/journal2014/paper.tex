...
Pre-processing the data takes approximately 12 minutes, 19 minutes and 40 minutes for 20\%, 40\% and 100\% of the genome, respectively.
For each of these sample sizes, the memory required doesn't exceed a modest 2GB per executor, even for all 22 autosomes. Memory requirements for clustering, however, increase in correlation with the dataset. 

Where our previous clustering of chromosome 22 required no more than 4GB memory per executor, when clustering the entire genome, this increases this requirement to over 20GB. This scales in a linear trend, where 20\% of the genome requires 6GB and 40\% of the genome requires 12GB. To cluster 100\% of the genome, we need to allocate
...
We completed the Chromosome 22 comparisons on a virtual machine (VM) hosted on Microsoft Azure. This VM is an A7 Linux instance with 8 cores, 56GB RAM and Ubuntu installed as the OS. 
...
This results in KVPs where the key is the individual ID, and the value is a tuple of the allele and it's index. We effectively have one KVP for each allele from the VCF file. At this point, each allele is still a string straight from the VCF file (i.e. `0\textbar1'), however, we require doubles.
