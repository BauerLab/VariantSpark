...
\subsection*{Hardware}
We processed the data using a cluster of VMs on the Microsoft Azure cloud service. Each of the VMs has 7GB memory and and 4 CPU cores.
Six of the VMs have the YARN NodeManager installed, and therefore these 6 machines process the data. The seventh machine dedicates the majority of its memory to
the HDFS NameNode, ResourceManager and monitoring services. An eighth, single core machine, hosts the Cloudera manager service.
With the 6 quad-core machines, we can run a total of 24 YARN containers simultaneously.
