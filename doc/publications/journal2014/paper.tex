...
For each of these sample sizes, the memory required doesn't exceed a modest 2GB per executor, even for the entire autosome. Memory requirements for clustering, however, increase in correlation with the dataset. 

Where our previous clustering of chromosome 22 required no more than 4GB memory per executor, clustering the entire genome increases this requirement to over 20GB. This scales in a linear trend, where 20\% of the genome requires 6GB and 40\% of the genome requires 12GB. To cluster 100\% of the genome, we need to allocate
at least 24GB to each executor. 
...
With Dense Vectors, the zero values (which are excluded from Sparse Vectors so don't require any memory) are explicitly defined, considerably increasing the memory required. 
...
We used our previous Hadoop implementation based on the MapReduce model. This makes use of KVPs, similar to \variantSpark\ however, it is explicitly based on the MapReduce model.
The Map stage creates keys from the VCF file, where the key is a tuple of a primary and secondary key. The primary key is the individual ID and the secondary key is the allele ID. The value is the variant.
The primary key ensures that KVPs for each individual are distributed to the same node during the MapReduce shuffle stage. The KVPs for each individual are then sorted by their secondary key.
Finally, the Reduce stage creates a sparse vector for each individual from the individual's KVPs.
