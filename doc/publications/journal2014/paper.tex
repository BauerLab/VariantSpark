...
In this section we demonstrate Mahout's scaling ability by increasing the number of features (variants per sample) to include the entire phase3 autosome dataset (\FullPhasethree{}).
We focus on the \FullPhasethree{} dataset in this section because it has 79 million variants compared to \FullPhaseone{}'s 38 million.

We start by clustering one chromosome. Our algorithm completes successfully, however, it only achieves an adjusted Rand index of 0.622.
Increasing the dataset to include the first three chromosomes (\ThreePhasethree{}) results in an adjusted Rand index on 0.825
Finally we include all 22 autosomes as input to our algorithm (\FullPhasethree{}). Although the adjusted rand index isn't much higher than the previous example, at 0.84, the pre-processing and k-means complete successfully.

% This section feels like a listing numbers rather than a "story". Maybe having a plot we can make a more general statement. So can you make a plot #features vs memory and in a separate on vs time with different colours showing pre-post processing. Or one plot memory vs time with colours as features and shape as pre-post ?
...
%
%% mentioning phase 3 here makes it difficult to understand.
...
%
%%DB how else would you expect memory to grow 
...
In this section we investigate Mahout's ability to scale with more samples. Mahout's k-means algorithm performs by splitting the clustering job up into separate tasks, where each task calculates the distance between a subset of samples and the k-means centroids.
Mahout performs each of these calculations iteratively until they are complete for every sample.
In the previous section we increased the sample size. Larger samples have and increased dimensionality, and hence require more memory and also more time to calculate the distances.
In this case however, we increase the number of samples. By simply increasing the number of samples, the k-means algorithm will perform more iterations, however the resource cost of each iteration should remain the same.
To demonstrate this, we compare pre-processing and clustering of the 2504 individuals from the phase3 dataset compared to the 1092 individuals in the phase1 dataset.
We specifically use the \SevenPhaseone{} and \ThreePhasethree{} datasets because of their similar feature size.
Even with twice the number of samples in the larger dataset, the algorithm completed successfully for both datasets with the same container size.
However the \ThreePhasethree{} dataset did require about twenty minutes more time than the smaller \SevenPhaseone{} dataset.
This demonstrates Hadoop's strength of scaling with sample size. Rather than the larger comparison requiring more resources, it simple required more time. The container size however, cannot be an arbitrary value, as we discuss in the next section.
