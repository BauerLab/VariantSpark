...
We compare the time required to cluster individuals based on their chromosome 22 variants using various implementations. We compare our {\sc VariantSpark} implementation with ADAM and our Hadoop implementation, as well as a Python and R implementation.
...
Pre-processing the data is fastest in {\sc VariantSpark}, requiring 2mins and 58secs. Next fastest is the ADAM implementation, requiring 12min and 48secs. Our Hadoop implementation took 14min and 22secs.
Unlike {\sc VariantSpark} and our Hadoop implementation, which pre-process VCF files directly, ADAM requires the input to be in their ADAM file format. VCF files must be converted to this binary format before they can be pre-processed into the row vector format. Although this additional pre-processing step is only required as a one-off for each input file, it requires an additional 13mins and 13secs.

Clustering the samples is also fastest in {\sc VariantSpark}, albeit by a smaller margin. The time required is 1min and 20secs compared to 1min and 52secs, in {\sc VariantSpark} and ADAM, respectively. Hadoop, however, required 14mins and 23secs.
The similarity between {\sc VariantSpark} and ADAM is expected, as they both use {\sc Spark}'s MLlib k-means implementation. The 30 second speed increase in {\sc VariantSpark} is likely due to {\sc VariantSpark}'s pre-processing implementation converting VCF files to sparse vectors, whereas ADAM creates dense vectors.
Due to the sparse nature of these row-vectors, the sparse vector format is likely to reduce some of the initial overhead.

We also compared k-means clustering times to a multi-threaded approach in Python and R. Although slower than the {\sc Spark} implementations in {\sc VariantSpark} and ADAM, k-means clustering in both Python and R were faster than our Hadoop implementation.
Python required 11min and 29sec, whereas R required 7min and 25sec. This demonstrates one of Hadoop's disadvantages (one which {\sc Spark} dramatically improves on): the cost of disk IO.
Where Hadoop's advantage is that it can scale to datasets that exceed memory size, in this example, the entire dataset can fit in memory on a single machine.
So, where the k-means implementations we use in R and Python store the output from each k-means iteration in-memory, Hadoop, writes the output of each iteration to disk, whether or not the amount of available memory is sufficient to store this data.
This unnecessary disk IO forms a bottleneck that results in the longer run-times for Mahout's k-means implementation. {\sc Spark}, on the other hand, will cache the intermediate results in memory, virtually eliminating the disk IO bottleneck on smaller datasets that fit entirely in memory, or at least reducing the disk IO by caching a portion of the data.

To summarise the total times for each big-data platform, {\sc VariantSpark} was 4mins and 18secs, ADAM was 14mins and 14secs (or 27mins and 53secs including the one-off conversion of VCF to ADAM) and our Hadoop implementation was 28mins and 45secs. 


\subsection*{{\sc VariantSpark} allows genome wide sampling of variants to improve clustering quality}

To demonstrate the scalability of {\sc Spark}, we continue to cluster individuals from the 1000 Genome Project, however, rather than clustering on a single chromosome, we cluster the individuals on their variants from all 22 autosomes (.
Although this pre-processing and clustering is successful, the memory required for clustering increases dramatically.
