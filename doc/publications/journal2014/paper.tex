...
For our R implementation, we utilize {\sc read.table} to read in a VCF file as a tab-delimited document. As this also captures the first nine columns that don't contain variants, we subsequently remove those columns with {\sc vcf[1:9] <- list()}.
...
In the supplementary material we benchmark a slightly faster (by less than one minute) approach of reading the VCF file in using the `VariantAnnotation' package.
